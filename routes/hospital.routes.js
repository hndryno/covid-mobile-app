...
const hospitalNoPaginate = require('../controller/mobile/hospital/list-no-paginate.controller')

//routes
router.get('/', auth, async (req, res, next) => await new getHospital().exec(req, res, next))

router.get('/no-paginate', auth, async (req, res, next) => await new hospitalNoPaginate().exec(req, res, next))
